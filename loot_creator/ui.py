...
def render_quick_mode() -> None:
    ...
    if st.button("Generate Item", type="primary", use_container_width=True, key="quick_generate"):
        ...
        user_api_key = st.session_state.get("openrouter_api_key", "")
...
def render_generated_item() -> None:
    ...
    if item.properties:
        ...
        for prop in item.properties:
            ...
            if ":" in prop:
                label, rest = prop.split(":", 1)
                items_li += f"<li><strong>{_esc(label.strip())}:</strong> {_esc(rest.strip())}</li>"
            else:
                items_li += f"<li>{_esc(prop)}</li>"
        props_html = f'<ul class="dnd-list">{items_li}</ul>'

    # --- Curse ---
    curse_html = ""
    if item.curse:
        curse_html = f'<div class="dnd-curse"><strong>Curse:</strong> {_esc(item.curse)}</div>'

    # --- Extras (gold, CR) ---
    extras_html = ""
    has_extras = item.gold_value or item.suggested_cr or item.crafting_materials
    if has_extras:
        extras_inner = ""
        if item.gold_value:
            extras_inner += f'<div><span>Market Value</span>{item.gold_value:,} gp</div>'
        if item.suggested_cr:
            extras_inner += f'<div><span>Typical Drop Source</span>{_esc(item.suggested_cr)}</div>'
        extras_html = f'<hr class="dnd-section-divider"><div class="dnd-extras">{extras_inner}</div>'

        if item.crafting_materials:
            mats_li = "".join(f"<li>{_esc(m)}</li>" for m in item.crafting_materials)
            extras_html += f'<div class="dnd-materials"><span style="font-family:\'Cinzel\',serif;font-size:13px;text-transform:uppercase;color:#7a5c3c;">Crafting Materials</span><ul>{mats_li}</ul></div>'

    # --- Lore ---
    lore_html = ""
    if item.lore:
        lore_html = f'<hr class="dnd-section-divider"><p class="dnd-lore">{_esc(item.lore)}</p>'

    subtitle = f"{_esc(item.subtype)} ({_esc(item.item_type)}) &mdash; {_esc(item.rarity)}"

    # Build HTML as a single flat string — no leading spaces so markdown never
    # misinterprets indented lines as code blocks
    card_html = (
        "<div class=\"dnd-card\">"
        f"<div class=\"dnd-title\">{_esc(item.name)}</div>"
        f"<div class=\"dnd-subtitle\">{subtitle}</div>"
        "<hr class=\"dnd-divider\">"
        + attunement_html
        + f"<p class=\"dnd-description\">{_esc(item.description)}</p>"
        + props_html
        + curse_html
        + extras_html
        + lore_html
    ...
    # Inject CSS first in a separate call, then the card HTML
